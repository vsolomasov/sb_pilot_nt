...
JNIEXPORT jint JNICALL Java_ru_kinoplan_sbrf_ISbrfNative_rollbackLastTransaction(JNIEnv *jEnv, jobject jobj, jint jAmount) {
	int amount = (int)jAmount;
	Pilot_NT_JNI *pilot = new Pilot_NT_JNI();
	int res = pilot->rollbackLastTrx(amount);
	delete pilot;
	pilot = 0;
	return res;
}
...
JNIEXPORT jint JNICALL Java_ru_kinoplan_sbrf_ISbrfNative_abort(JNIEnv *jenv, jobject jobj) {
	Pilot_NT_JNI *pilot = new Pilot_NT_JNI();
	int res = pilot->abort();
	delete pilot;
	pilot = 0;
